      ...
      <div className="px-4 mt-3">
        <div className="flex items-center gap-2">
          <div className="flex-1 flex items-center bg-neutral-100 rounded-full h-11 px-4 gap-2 transition-colors focus-within:bg-white focus-within:ring-2 focus-within:ring-emerald-100 focus-within:border focus-within:border-emerald-300">
            ...
            <input data-testid="search-input" autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search for honey, oil, spices…" className="flex-1 bg-transparent outline-none text-sm" />
          </div>
        ...
        {showFilter && (
          <div className="mt-3 rounded-2xl bg-white border border-neutral-200 p-4 space-y-4 animate-in slide-in-from-top-2 duration-200">
            <div>
              <div className="text-[11px] uppercase tracking-wider font-semibold text-neutral-600 mb-1.5">Category</div>
              <div className="flex flex-wrap gap-1.5">
                <button data-testid="filter-cat-all" onClick={() => setCat('')} className={`px-3 h-8 rounded-full text-[12px] font-semibold border transition-colors ${!cat ? 'bg-emerald-700 border-emerald-700 text-white' : 'bg-white border-neutral-200 text-neutral-700 hover:border-emerald-400'}`}>All</button>
                {cats.map((c) => (
            ...
            <div>
              <div className="text-[11px] uppercase tracking-wider font-semibold text-neutral-600 mb-1.5">Price range (৳)</div>
              <div className="grid grid-cols-2 gap-2">
                <input data-testid="filter-price-min" type="number" value={price.min} onChange={(e) => setPrice({ ...price, min: e.target.value })} placeholder="Min" className="h-10 px-3 rounded-xl bg-neutral-50 border border-neutral-200 outline-none focus:border-emerald-500 text-sm" />
                <input data-testid="filter-price-max" type="number" value={price.max} onChange={(e) => setPrice({ ...price, max: e.target.value })} placeholder="Max" className="h-10 px-3 rounded-xl bg-neutral-50 border border-neutral-200 outline-none focus:border-emerald-500 text-sm" />
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              <label className="inline-flex items-center gap-2 text-sm cursor-pointer">
                <input data-testid="filter-organic" type="checkbox" checked={organic} onChange={(e) => setOrganic(e.target.checked)} className="w-4 h-4 accent-emerald-600" /> Organic only
              </label>
              <label className="inline-flex items-center gap-2 text-sm cursor-pointer">
                <input data-testid="filter-instock" type="checkbox" checked={inStock} onChange={(e) => setInStock(e.target.checked)} className="w-4 h-4 accent-emerald-600" /> In stock only
              </label>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-wider font-semibold text-neutral-600 mb-1.5">Sort by</div>
              <div className="flex flex-wrap gap-1.5">
                {SORTS.map((s) => (
            ...
            <div className="flex items-center justify-between pt-1 border-t border-neutral-100">
              <button data-testid="filter-clear-all" onClick={clearAll} className="inline-flex items-center gap-1 text-[12px] font-semibold text-neutral-600 hover:text-neutral-900 transition-colors"><X className="w-3 h-3" /> Clear all</button>
              <button data-testid="filter-apply" onClick={() => setShowFilter(false)} className="inline-flex items-center gap-1 bg-emerald-700 text-white text-[12.5px] font-semibold h-9 px-4 rounded-full hover:bg-emerald-800 transition-colors">Apply</button>
            </div>
          </div>
        )}

        <div className="text-[11px] text-neutral-500 mt-3">{results.length} product{results.length !== 1 ? 's' : ''}</div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-2">
          {results.map((p) => (<ProductCard key={p.id} product={p} />))}
        </div>
        {results.length === 0 && (
          <div className="text-center py-14 text-sm text-neutral-500">No products match your filters.</div>
        )}
      </div>
    </div>
  );
